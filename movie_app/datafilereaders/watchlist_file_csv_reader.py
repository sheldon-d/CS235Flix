...
class WatchListFileCSVReader:
    ...
    def __init__(self, file_name: str, movies: Iterable[Movie], users: Iterable[User]):
        ...
        self.__dataset_of_movies: Iterable[Movie] = list(movies)
        self.__dataset_of_users: Iterable[User] = list(users)
        ...
        with open(self.__file_name, mode='r', encoding='utf-8-sig') as csv_file:
            ...
            for row in watchlist_file_reader:
                ...
                if watchlist_user is not None and watchlist_user.id is not None:
                    watchlist_user.watchlist.user = watchlist_user

                    for val in row['Movie Ranks']:
                        try:
                            movie_rank = int(val)
                            ...
                            pass    # Ignore exception and don't add movie
